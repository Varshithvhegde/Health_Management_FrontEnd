...
//POST request
function doRequest(username, mail, pass, phone_number, designation) {
    fetch("http://127.0.0.1:3000/api/v1/register/medical_staff", {
        method: "POST",
        body: JSON.stringify({
            email: mail.value,
            name: username.value,
        ...
        .then(function (response) {
            if (response.ok) {
                error.innerText = "";
                window.location.assign("./doctor_login.html");
